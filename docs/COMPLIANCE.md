...
| Key management / rotation | ⚠ | **rotate all keys before go-live** (some were shared in chat). Connector-secret key is auto-provisioned to `platform_secrets` (DB) at soft launch for **non-PHI** connectors only — must move to `CONNECTOR_ENC_KEY` env / KMS before any PHI (see [PHI-INFRA-CHECKLIST.md](PHI-INFRA-CHECKLIST.md) §3). **Enforced:** with `PHI_GATE_READY=true` the DB-key fallback is refused for every `phiGated` provider (connect + sync), and the server fails to boot if PHI mode lacks `CONNECTOR_ENC_KEY` (`assertPhiKeyInvariant`). |
