...
Migration: `supabase/migrations/20260607070000_platform_secrets_immutable.sql`. To verify the guard on a fresh DB, paste `supabase/tests/platform_secrets_immutable.test.sql` into the SQL editor — it asserts the row is immutable and rolls back, leaving no data behind. (Verification tests live under `supabase/tests/`, not `supabase/migrations/`, so no migration runner ever applies them.)
